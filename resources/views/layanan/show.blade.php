@extends('layouts.app')
@section('title', 'Detail Layanan — Aluna')
@section('content')
<div x-data="layananDetailPage()" x-init="init()">
    <div class="page-banner flex items-center gap-3 mb-6">
        <a href="/layanan" class="text-gray-600 hover:text-gray-800">
    ...
    <template x-if="loading">
        <div class="rounded-3xl border border-gray-100 bg-white p-8 text-center text-gray-500">Memuat layanan...</div>
    </template>

    <template x-if="!loading && !item">
        <div class="rounded-3xl border border-gray-100 bg-white p-8 text-center text-gray-500">Layanan tidak ditemukan.</div>
    </template>

    <template x-if="item">
        <div class="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            <template x-if="item.gambar_artikel">
                <img :src="'/storage/' + item.gambar_artikel" class="w-full rounded-3xl object-cover mb-6" alt="Gambar layanan">
            </template>
            ...
            <template x-if="item.gambar_artikel_2">
                <img :src="'/storage/' + item.gambar_artikel_2" class="w-full rounded-3xl object-cover mt-6" alt="Gambar layanan 2">
            </template>
        </div>
    </template>
</div>
