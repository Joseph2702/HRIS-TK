...
@section('content')
    <div x-data="layananPage()" x-init="init()">
        ...
        <div class="page-banner">
            <h1>Layanan yang tersedia di Sekolah TK Aluna Montessori</h1>
        </div>
        ...
        <div class="mb-5 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm flex items-center gap-3">
            ...
            <div class="flex items-center gap-3">
                <a href="https://wa.me/6281234567890?text=Halo%20Admin%2C%20saya%20ingin%20bertanya%20tentang%20anak%20saya"
                    target="_blank" rel="noopener noreferrer"
                    class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-white shadow-sm"
                        ...
                        <tbody>
                            <template x-for="app in appointments" :key="app.id_appointment">
                                <tr class="border-t border-gray-50">
                                    <td class="py-3 pr-3">
                                        <div class="font-medium text-gray-800 line-clamp-2" x-text="app.reason || '-'">
                                        </div>
                                    </td>
                                    <td class="py-3 pr-3 text-gray-700"
                                        x-text="app.from_date || app.fromDate || app.from || '-'"></td>
                                    <td class="py-3 pr-3 text-gray-700" x-text="parseTimeFromReason(app.reason) || '-'">
                                    </td>
            ...
            <template x-for="item in filtered" :key="item.id_artikel">
                <div class="bg-white rounded-xl p-4 flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="mt-2">
                            <a :href="'/layanan/'+item.id_artikel" class="text-xs text-blue-600 hover:underline">Lihat
                                detail</a>
                        </div>
                    </div>
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto"
                ...
                <form @submit.prevent="submitAppointment()" class="space-y-4">
                    <div>
                        ...
                        <textarea x-model="consultForm.topik" class="input h-28 resize-none" style="border-radius:1rem"
                            required></textarea>
                    </div>
        ...
        <div x-show="showModal" class="fixed inset-0 z-40 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto"
                ...
                <form @submit.prevent="save()" class="space-y-4">
                    <div><label class="label">Judul Layanan</label><input type="text" x-model="form.judul_artikel"
                            class="input" required></div>
                    <div><label class="label">Deskripsi</label><textarea x-model="form.konten_artikel"
                            class="input h-28 resize-none" style="border-radius:1rem" required></textarea></div>
                    <div>
                        <label class="label">Foto 1 <span class="text-gray-400 font-normal">(maks. 100 MB)</span></label>
                        <input type="file" @change="onFoto1($event)"
                            class="w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-medium file:cursor-pointer"
                            accept="image/*">
                        <template x-if="foto1Preview"><img :src="foto1Preview"
                                class="mt-2 h-20 rounded-xl object-cover"></template>
                    </div>
                    <div>
                        <label class="label">Foto 2 <span class="text-gray-400 font-normal">(opsional)</span></label>
                        <input type="file" @change="onFoto2($event)"
                            class="w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-medium file:cursor-pointer"
                            accept="image/*">
                        <template x-if="foto2Preview"><img :src="foto2Preview"
                                class="mt-2 h-20 rounded-xl object-cover"></template>
                    </div>
                    <div class="flex justify-end gap-3 pt-2">
        ...
        <a href="https://wa.me/6281234567890?text=Halo%20Admin%2C%20saya%20ingin%20bertanya%20tentang%20anak%20saya"
            target="_blank" rel="noopener noreferrer" aria-label="Hubungi Sekolah via WhatsApp"
